...
public sealed class PathEvaluationTest
{
    ...
    [Fact]
    public void Repeat_FolderParent_AncestorMatch_WithTrailingFilter()
    {
        using var env = Environment.CreateDatabase(dbName: DatabaseCollection.GetTempDbDirectory(), dumpFile: DatabaseCollection.GetBusinessModelDumpFile());
        using var session = new DbSession(env);
        ...
        var root = new Folder(session) { Name = "Root" };
        var a = new Folder(session) { Name = "A", Parent = root };
        var b = new Folder(session) { Name = "B", Parent = a };

        var otherRoot = new Folder(session) { Name = "OtherRoot" };
        var other = new Folder(session) { Name = "Other", Parent = otherRoot };

        var src = "P(Folder): repeat(this->Parent)[$.Name=\"Root\"]";
        var parse = PathLangParser.Parse(src);
        Assert.DoesNotContain(parse.Diagnostics, d => d.Severity == PathLangDiagnosticSeverity.Error);
        ...
        Assert.True(PathEvaluation.Evaluate(session, root.ObjId, predicate, bind.SemanticModel));
        Assert.True(PathEvaluation.Evaluate(session, a.ObjId, predicate, bind.SemanticModel));
        Assert.True(PathEvaluation.Evaluate(session, b.ObjId, predicate, bind.SemanticModel));
        Assert.False(PathEvaluation.Evaluate(session, other.ObjId, predicate, bind.SemanticModel));
    }

    [Fact]
    public void Repeat_IsChainable_RepeatThenTraverse()
    {
        using var env = Environment.CreateDatabase(dbName: DatabaseCollection.GetTempDbDirectory(), dumpFile: DatabaseCollection.GetBusinessModelDumpFile());
        using var session = new DbSession(env);
        ...
        var src = "P(Folder): repeat(this->Parent)->Subfolders[$.Name=\"Finance\"]";
        var parse = PathLangParser.Parse(src);
        Assert.DoesNotContain(parse.Diagnostics, d => d.Severity == PathLangDiagnosticSeverity.Error);
    ...
    [Fact]
    public void Repeat_DoesNotInfiniteLoop_OnCycles()
    {
        using var env = Environment.CreateDatabase(dbName: DatabaseCollection.GetTempDbDirectory(), dumpFile: DatabaseCollection.GetBusinessModelDumpFile());
        using var session = new DbSession(env);
